...
class EpreuvesController extends BaseController
{
	...
	{
		try {
			$epreuve = Epreuve::findOrFail($epreuveId); //TODO: catcher ModelNotFoundException
	...
	{
		$epreuve = Epreuve::findOrFail($epreuveId); //TODO catcher les exceptions
		return View::make('Epreuves.show', compact('epreuve')); //TODO faire le show	
	}	
	...
	{
		$input = Input::all();
		$sport = Sport::findOrFail($input["sportsListe"]); //TODO: catcher ModelNotFoundException
		...
		if($epreuve->save()) {
			...
			$epreuve = $sport->epreuves()->save($epreuve); //TODO gerer les exceptions
	...
	public function epreuvesPourSportDropdown() {
		if(Request::ajax()) {
			$sportId = Input::get('sportId');
			try {
				$sport = Sport::findOrFail($sportId);
			...
			$epreuves = $sport->epreuves;
			return View::make('epreuves.dropdownEpreuve')->with('epreuves',$epreuves)->with('sport',$sport);
